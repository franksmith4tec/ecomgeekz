...
// If no transaction_id → show 404
if (!isset($_GET['transaction_id']) || $_GET['transaction_id'] < 0) {
    header("HTTP/1.0 404 Not Found");
    header("Location: " .$payment_config['base_url']. "/404.php");
    exit;
}

$transaction_id = mysqli_real_escape_string($conn, $_GET['transaction_id']);
// Validate against payment_detail table
$sql = "SELECT * FROM payment_detail WHERE authorize_transaction_id = '$transaction_id' LIMIT 1";
$result = mysqli_query($conn, $sql);

// If no matching record → show 404
...
<head>
    ...
    <title><?= $brand_name ?? '' ?> | Thank You</title>
        ...
        <div class="container">
            <div class="row">
                <div class="row ftr_btm">
                    ...
                    <div class="col-md-3">
                        <span>© Copyright 2025 <?= $brand_name ?? '' ?></span>
                    </div>
                </div>
            </div>
        </div>
